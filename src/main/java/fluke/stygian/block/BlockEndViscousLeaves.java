...
public class BlockEndViscousLeaves extends BlockLeaves {
    public static final String REG_NAME = "endviscousleaves";

    public BlockEndViscousLeaves() {
        this.setDefaultState(this.blockState.getBaseState()
                .withProperty(CHECK_DECAY, Boolean.FALSE)
                .withProperty(DECAYABLE, Boolean.TRUE));

        setTranslationKey(Reference.MOD_ID + ".endviscousleaves");
    ...
    public void updateTick(World worldIn, BlockPos pos, IBlockState state, Random rand) {
        if (!worldIn.isRemote) {
            if (state.getValue(CHECK_DECAY) && state.getValue(DECAYABLE)) {
                ...
                if (worldIn.isAreaLoaded(new BlockPos(x - checkRange, y - checkRange, z - checkRange), new BlockPos(x + checkRange, y + checkRange, z + checkRange))) {
                    ...
                    for (int dx = -range; dx <= range; ++dx) {
                        for (int dy = -range; dy <= range; ++dy) {
                            for (int dz = -range; dz <= range; ++dz) {
                                ...
                                if (iblockstate.getBlock() == ModBlocks.endViscousLog || iblockstate.getBlock().canSustainLeaves(iblockstate, worldIn, mutablePos)) {
    ...
    public Item getItemDropped(IBlockState state, Random rand, int fortune) {
        return Item.getItemFromBlock(ModBlocks.endViscousSapling);
    }
