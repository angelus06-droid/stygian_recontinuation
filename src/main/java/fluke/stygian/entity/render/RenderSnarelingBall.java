...
public class RenderSnarelingBall extends Render<EntitySnarelingBall>
{
    private final float scale;

    public RenderSnarelingBall(final RenderManager renderManagerIn, final float scaleIn) {
        super(renderManagerIn);
        this.scale = scaleIn;
    }

    public void doRender(final EntitySnarelingBall entity, final double x, final double y, final double z, final float entityYaw, final float partialTicks) {
        GlStateManager.pushMatrix();
        this.bindEntityTexture(entity);
        GlStateManager.translate((float)x, (float)y, (float)z);
        GlStateManager.enableRescaleNormal();
        GlStateManager.scale(this.scale, this.scale, this.scale);
        final TextureAtlasSprite textureatlassprite = Minecraft.getMinecraft().getRenderItem().getItemModelMesher().getParticleIcon(ModBlocks.endViscousBall);
    ...
    protected ResourceLocation getEntityTexture(final EntitySnarelingBall entity) {
