...
public class ChunkGeneratorEndBiomes extends ChunkGeneratorEnd {
                ...
                for (int k1 = 0; k1 < 32; ++k1) {
                    ...
                    for (int l1 = 0; l1 < 4; ++l1)
                    {
                        ...
                        for (int i2 = 0; i2 < 8; ++i2)
                        {
                            ...
                            {
                                IBlockState iblockstate = (d15 > 0.0D) ? END_STONE : AIR;
                                primer.setBlockState(i2 + i1 * 8, l1 + k1 * 4, j2 + j1 * 8, iblockstate);
                                d15 += d16;
                            }
                            d10 += d12;
                            d11 += d13;
    ...
    public void buildSurfaces(ChunkPrimer primer, int chunkX, int chunkZ, Biome[] biomeMap) {
        ...
        for (int i = 0; i < 16; ++i) {
            for (int j = 0; j < 16; ++j) {
                ...
                IBlockState surface = (current != Biomes.SKY) ? current.topBlock : END_STONE;
                IBlockState filler = (current != Biomes.SKY) ? current.fillerBlock : END_STONE;
                ...
                for (int i1 = 127; i1 >= 0; --i1) {
                    ...
                    if (iblockstate2.getMaterial() == Material.AIR) {
                        ...
                        if (l == -1) {
                            l = 3 + rand.nextInt(2);
                            primer.setBlockState(i, i1, j, (i1 >= 0) ? surface : filler);
                        }
                        else if (l > 0) {
                            --l;
    ...
    private double[] getHeights(double[] p_185963_1_, int p_185963_2_, int p_185963_3_, int p_185963_4_, int p_185963_5_, int p_185963_6_, int p_185963_7_)
    {
        ...
        for (int l = 0; l < p_185963_5_; ++l)
        {
            ...
            {
                float f = this.getIslandHeightValue(p_185963_2_ / 2, p_185963_4_ / 2, l, i1);
                for (int j1 = 0; j1 < p_185963_6_; ++j1)
                {
                    double d5 = (this.pnr[k] / 10.0D + 1.0D) / 2.0D;
                    double d4 = (d5 < 0.0D) ? ar[k]/512.0D : (d5 > 1.0D ? br[k]/512.0D : (ar[k]/512.0D + (br[k]/512.0D - ar[k]/512.0D) * d5));
                    d4 = d4 - 8.0D + (double)f;

                    if (j1 > p_185963_6_ / 2 - 2)
                    {
                        double d6 = MathHelper.clamp((double)((float)(j1 - (p_185963_6_ / 2 - 2)) / 64.0F), 0.0D, 1.0D);
                        d4 = d4 * (1.0D - d6) + -3000.0D * d6;
                    }
                    if (j1 < 8)
                    {
                        double d7 = (double)((float)(8 - j1) / 7.0F);
